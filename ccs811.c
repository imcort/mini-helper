...
//      goto abort_begin;
//    }
    nrf_delay_us(CCS811_WAIT_AFTER_RESET_US);
...
//    }
    if( status!=0x10 ) {
			NRF_LOG_INFO("ccs811: Not in boot mode, or no valid app: %x", status);
      goto abort_begin;
    }
...
bool CCS811_i2cwrite(uint8_t regaddr, int count, const uint8_t * buf) {
...
//  return r==0;
	...
	uint8_t sendbuf[10];
	sendbuf[0] = regaddr;
	memcpy(sendbuf+1, buf, count);
	
//	while (m_xfer_done == false);
//	m_xfer_done = false;
  err_code = nrf_drv_twi_tx(&m_twi, CCS811_ADDR, sendbuf, count+1, false);
...
bool CCS811_i2cread(uint8_t regaddr, int count, uint8_t * buf) {
//  Wire.beginTransmission(_slaveaddr);              // START, SLAVEADDR
//  Wire.write(regaddr);                             // Register address
